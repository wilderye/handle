...
const fontPath = join(
  __dirname,
  "../../src/assets/fonts/NotoSerifSC-Regular.otf"
);
try {
...
// 辅助函数：获取颜色
function getColor(
  state: "exact" | "misplaced" | "none" | undefined,
  allExact: boolean,
  isChar = false
): string {
 ...
 * 生成游戏面板截图
 */
export async function generateGameBoardScreenshot(
  data: GameBoardData
): Promise<Buffer> {
  ...
  data.tries.forEach((word, rowIndex) => {
    ...
    chars.forEach((char, charIndex) => {
      ...
      const w2 = p2 ? ctx.measureText(p2.replace("v", "ü")).width : 0;
 ...
 * 生成速查表截图
 */
export async function generateCheatsheetScreenshot(
  data: CheatsheetData
): Promise<Buffer> {
  // 声母
  const initials = [
  ...
  const height =
    padding * 2 +
    labelHeight +
    initialsRows * cellHeight +
    sectionGap +
    labelHeight +
    finalsRows * cellHeight;
  ...
  initials.forEach((s, i) => {
    ...
    if (state === "exact") ctx.fillStyle = COLORS.ok;
      ...
      ctx.fillStyle = "rgba(55, 65, 81, 0.3)"; // 已排除，淡色
